...
class DeploymentConfig:
    ...
    # AWS S3 Settings
    DOWNLOAD_SOURCE = "ssh"  # "ssh" or "s3"
    S3_BUCKET = "iflightdevrdits3"
    S3_PREFIX_TEMPLATE = "iFlight_Release/{version}/Wars/"
    S3_PROFILE = "iFlightCrew_Dev"
...
def load_config_from_json(config_path):
    ...
    # AWS S3 Settings
    config.DOWNLOAD_SOURCE = json_config.get('download_source', getattr(config, 'DOWNLOAD_SOURCE', 'ssh'))
    s3_cfg = json_config.get('s3_config', {})
    config.S3_BUCKET = s3_cfg.get('bucket', getattr(config, 'S3_BUCKET', 'iflightdevrdits3'))
    config.S3_PREFIX_TEMPLATE = s3_cfg.get('prefix_template', getattr(config, 'S3_PREFIX_TEMPLATE', 'iFlight_Release/{version}/Wars/'))
    config.S3_PROFILE = s3_cfg.get('profile', getattr(config, 'S3_PROFILE', 'iFlightCrew_Dev'))
    config.S3_REGION = s3_cfg.get('region', getattr(config, 'S3_REGION', 'ap-south-1'))
